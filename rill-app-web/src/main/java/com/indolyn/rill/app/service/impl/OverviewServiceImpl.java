...
public class OverviewServiceImpl implements OverviewService {
    ...
    public OverviewServiceImpl(DatabaseService databaseService) {
    ...
    private List<OverviewModuleResponse> buildModules() {
        return List.of(
            new OverviewModuleResponse("rill-core", "数据库内核", "不直接暴露为 Web 或 GUI 入口",
                "承载 SQL 编译、执行、catalog、表堆、索引、事务、恢复。"),
            new OverviewModuleResponse("rill-server", "服务端外壳", "数据库主体安装包的一部分",
                "提供原生服务端与 MySQL/Navicat 兼容服务端。"),
            new OverviewModuleResponse("rill-client", "本地客户端", "可选组件", "提供 CLI、GUI 与本地工具入口。"),
            new OverviewModuleResponse("rill-app-web", "Spring Boot 演示台后端", "独立 Web jar",
                "把内核能力变成浏览器里可讲解、可观察、可展示的演示界面。"),
            new OverviewModuleResponse("web", "Vue 演示台前端", "独立前端或嵌入式 UI",
                "围绕 SQL、trace、架构、能力摘要和扩展路线组织信息。"));
    }

    private List<OverviewCapabilityResponse> buildCapabilities() {
        return List.of(
            new OverviewCapabilityResponse("SQL", "Workbench 执行 SQL", "可以直接执行 SQL，展示结果、错误、耗时和 trace。"),
            ...
            new OverviewCapabilityResponse("Ops", "多平台构建与发布", "CI、Release、安装包和独立 Web jar 已具备基础工程化形态。"));
    }
