...
public class MysqlProtocolHandler implements Runnable {
    ...
    public MysqlProtocolHandler(
        Socket clientSocket, QueryProcessor queryProcessor, Catalog catalog, int connectionId) {
        this.clientSocket = clientSocket;
    ...
    public void run() {
        ...
        try (InputStream in = clientSocket.getInputStream();
            ...
            System.out.println(
                "User '"
                    + session.getUsername()
                    + "' authenticated successfully for connection ID: "
                    + connectionId);
            ...
            if (!e.getMessage().contains("Connection reset")
                ...
                System.err.println(
                    "Error handling client connection " + connectionId + ": " + e.getMessage());
            }
        } catch (Exception e) {
            System.err.println(
                "Unexpected error in protocol handler " + connectionId + ": " + e.getMessage());
            e.printStackTrace();
        } finally {
            try {
                ...
                System.out.println(
                    "Client disconnected: " + session.getUsername() + "@" + clientSocket.getInetAddress());
            } catch (IOException e) {
                // Ignore
            }
        }
    ...
    private boolean authenticate(Packet authPacket, byte[] salt, OutputStream out)
        ...
        String username =
            new String(authPacket.payload, userStart, userEnd - userStart, StandardCharsets.UTF_8);
    ...
    private byte[] sendHandshake(OutputStream out, int sequenceId) throws IOException {
        ...
        packet.write(10);
        String serverVersion = "8.0.28-rill";
    ...
    private void handleCommand(Packet commandPacket, OutputStream out) throws Exception {
        ...
        switch (commandType) {
            ...
            case 0x02: // COM_INIT_DB (USE database)
                String dbName =
                    new String(
                        commandPacket.payload, 1, commandPacket.payload.length - 1, StandardCharsets.UTF_8);
                try {
                    switchDatabase(dbName);
                    sendOkPacket(out, serverSequenceId, 0, 0);
                } catch (Exception e) {
                    sendErrorPacket(
                        out, serverSequenceId, 1049, "42000", "Unknown database '" + dbName + "'");
                }
                break;
            case 0x03: // COM_QUERY
                String sql =
                ...
                if (sqlLower.startsWith("use ")) {
                    ...
                    try {
                        ...
                        sendErrorPacket(
                            out, serverSequenceId, 1049, "42000", "Unknown database '" + dbNameFromQuery + "'");
                    }
                    return; // 立即返回，不继续处理
                }
                if (queryProcessor == null
                    && !(sql.toLowerCase().contains("create database")
                    || sql.toLowerCase().contains("show databases"))) {
                ...
                try {
                    ...
                    System.err.println(
                        "Unsupported query syntax from client, returning empty result set. SQL: " + sql);
    ...
    private boolean handleSpecialQuery(String sql, OutputStream out, int sequenceId)
        throws IOException {
        String sqlLower = sql.toLowerCase().trim();
        ...
        if (sqlLower.contains("from information_schema.schemata")) {
            System.out.println(
                "Intercepted information_schema.schemata query. Returning real database list.");
    ...
    private int sendSimpleFieldPacket(OutputStream out, int sequenceId, String fieldName)
        throws IOException {
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
    ...
    private int sendResultSetHeader(OutputStream out, int sequenceId, int fieldCount)
        throws IOException {
        return writePacket(out, writeLengthEncodedInt(fieldCount), sequenceId);
    }

    private int sendFieldPackets(OutputStream out, int sequenceId, Schema schema, String tableName)
        throws IOException {
    ...
    private int sendRowPackets(OutputStream out, int sequenceId, List<Tuple> tuples, Schema schema)
        throws IOException {
    ...
    private int sendOkPacket(OutputStream out, int sequenceId, int affectedRows, int lastInsertId)
        throws IOException {
        ByteArrayOutputStream packet = new ByteArrayOutputStream();
        packet.write(0x00);
    ...
    private int sendErrorPacket(
        OutputStream out, int sequenceId, int errorCode, String sqlState, String message)
        throws IOException {
        ByteArrayOutputStream packet = new ByteArrayOutputStream();
        packet.write(0xFF);
    ...
    private boolean handleShowVariables(String sql, OutputStream out, int sequenceId)
        throws IOException {
        sendResultSetHeader(out, sequenceId, 2);
        sendSimpleFieldPacket(out, sequenceId + 1, "Variable_name");
        sendSimpleFieldPacket(out, sequenceId + 2, "Value");
    ...
    private boolean handleShowEngines(OutputStream out, int sequenceId) throws IOException {
        ...
        bos.write(
            writeLengthEncodedString("Supports transactions, row-level locking, and foreign keys"));
    ...
    private boolean handleSystemVariables(String sql, OutputStream out, int sequenceId)
        throws IOException {
        sendResultSetHeader(out, sequenceId, 1);
    ...
    private boolean handleInformationSchema(String sql, OutputStream out, int sequenceId)
        throws IOException {
    ...
    private boolean handleShowFullTables(OutputStream out, int sequenceId) throws IOException {
        ...
        sequenceId =
            sendSimpleFieldPacket(
                out, sequenceId, "Tables_in_" + (this.currentDb != null ? this.currentDb : ""));
    ...
    private boolean handleInformationSchemaColumns(String sql, OutputStream out, int sequenceId)
        throws IOException {
    ...
    private boolean handleInformationSchemaTables(String sql, OutputStream out, int sequenceId)
        throws IOException {
    ...
    private boolean handleShowCreateTable(String sql, OutputStream out, int sequenceId)
        throws IOException {
    ...
    private boolean handleShowFullColumns(String sql, OutputStream out, int sequenceId)
        throws IOException {
