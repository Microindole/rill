...
public class InteractiveShell {
  ...
  public static void main(String[] args) {
    ...
    String defaultUsername = options.username();

    System.out.println("Attempting to connect to rill server at " + host + ":" + port + "...");
  ...
  private static void sendCommand(String command, PrintWriter out, BufferedReader in)
      throws IOException {
    out.println(command);
    String serverResponse = in.readLine();
    if (serverResponse != null) {
  ...
  private record CliOptions(String host, int port, String username) {
    private static CliOptions parse(String[] args) {
      String host = firstNonBlank(System.getenv("RILL_HOST"), "localhost");
      int port = parseIntOrDefault(System.getenv("RILL_PORT"), 8848);
      String username = System.getenv("RILL_USER");
      ...
      if (args != null) {
        for (String arg : args) {
          ...
          if (arg.startsWith("--host=")) {
            ...
            port = parseIntOrDefault(arg.substring("--port=".length()), port);
      ...
      return new CliOptions(host, port, username);
    }

    private static String firstNonBlank(String candidate, String defaultValue) {
      return candidate == null || candidate.isBlank() ? defaultValue : candidate.trim();
    }
