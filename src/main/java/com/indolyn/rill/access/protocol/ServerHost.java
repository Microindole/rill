...
public class ServerHost {
    public static void main(String[] args) throws Exception {
        ...
        int port = parsePort(args, 8848);
        ServerSocket serverSocket = new ServerSocket(port);
        System.out.println("rill server started. Listening on port " + port + "...");
                        ...
                        while ((sql = in.readLine()) != null) {
                            ...
                            if (trimmedSql.startsWith("use ")) {
                                ...
                                if (trimmedSql.startsWith("create user") || trimmedSql.startsWith("grant")) {
                                    ...
                                    if (trimmedSql.startsWith("grant")) {
                                        try {
                                            ...
                                            if (ast instanceof GrantStatementNode grantNode) {
                                                ...
                                                if (!tableName.equals("*")
                                                    && currentQueryProcessor.getCatalog().getTable(tableName)
                                                    == null) {
                                                    // 如果表不存在，立即返回错误，不继续执行
                                                    out.println(
                                                        "ERROR: Table '"
                                    ...
                                    String result =
                                        defaultProcessor.executeAndGetResult(processorToUseSql, session);
                                    out.println(result.replace("\n", "<br>"));

                                } else {
                                    // 对于所有其他命令，使用当前处理器
                                    String result =
                                        currentQueryProcessor.executeAndGetResult(processorToUseSql, session);
                                    out.println(result.replace("\n", "<br>"));
                                }
                            }
                        }
    ...
    private static int parsePort(String[] args, int defaultPort) {
        ...
        String envPort = System.getenv("RILL_PORT");
