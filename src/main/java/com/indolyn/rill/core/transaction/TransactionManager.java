...
public class TransactionManager {
    ...
    public Transaction begin() throws IOException {
        ...
        // --- 写入 BEGIN 日志 ---
        LogRecord beginLog =
            new LogRecord(txn.getTransactionId(), txn.getPrevLSN(), LogRecord.LogType.BEGIN);
    ...
    public void commit(Transaction txn) throws IOException {
        // --- 写入 COMMIT 日志并刷盘 ---
        LogRecord commitLog =
            new LogRecord(txn.getTransactionId(), txn.getPrevLSN(), LogRecord.LogType.COMMIT);
    ...
    public void abort(Transaction txn) throws IOException {
        ...
        // --- 写入 ABORT 日志 ---
        LogRecord abortLog =
            new LogRecord(txn.getTransactionId(), txn.getPrevLSN(), LogRecord.LogType.ABORT);
