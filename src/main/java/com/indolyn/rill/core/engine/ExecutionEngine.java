...
public class ExecutionEngine {
  ...
  private TupleIterator buildExecutorTree(PlanNode plan, Transaction txn)
    ...
    if (plan instanceof InsertPlanNode insertPlan) {
      TableHeap tableHeap =
          new TableHeap(bufferPoolManager, insertPlan.getTableInfo(), logManager, lockManager);
    ...
    if (plan instanceof SeqScanPlanNode seqScanPlan) {
      TableHeap tableHeap =
          new TableHeap(bufferPoolManager, seqScanPlan.getTableInfo(), logManager, lockManager);
      AbstractPredicate predicate = null;
      if (seqScanPlan.getPredicate() != null) {
        predicate =
            createPredicateFromAst(seqScanPlan.getPredicate(), seqScanPlan.getOutputSchema());
      }
      // 将谓词（可能为 null）传递给执行器
    ...
    if (plan instanceof DeletePlanNode deletePlan) {
      ...
      TableHeap tableHeap =
          new TableHeap(bufferPoolManager, deletePlan.getTableInfo(), logManager, lockManager);
    ...
    if (plan instanceof UpdatePlanNode updatePlan) {
      ...
      TableHeap tableHeap =
          new TableHeap(bufferPoolManager, updatePlan.getTableInfo(), logManager, lockManager);
    ...
    if (plan instanceof FilterPlanNode filterPlan) {
      ...
      AbstractPredicate predicate =
          createPredicateFromAst(
              filterPlan.getPredicate(), filterPlan.getChild().getOutputSchema());
    ...
    if (plan instanceof ProjectPlanNode projectPlan) {
      ...
      for (String colName :
          projectPlan.getOutputSchema().getColumns().stream()
              .map(c -> c.getName())
    ...
    if (plan instanceof CreateIndexPlanNode createIndexPlan) {
      TableHeap tableHeap =
          new TableHeap(bufferPoolManager, createIndexPlan.getTableInfo(), logManager, lockManager);
    ...
    if (plan instanceof IndexScanPlanNode indexScanPlan) {
      TableHeap tableHeap =
          new TableHeap(bufferPoolManager, indexScanPlan.getTableInfo(), logManager, lockManager);
    ...
    if (plan instanceof CreateTablePlanNode createTablePlan) {
      return new CreateTableExecutor(
          createTablePlan, catalog, txn, logManager, bufferPoolManager, lockManager);
    }
