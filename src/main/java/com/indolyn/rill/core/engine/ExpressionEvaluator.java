...
/** 表达式求值器。 负责计算 WHERE 子句中的表达式对于给定的元组是否为真。 */
public class ExpressionEvaluator {
  ...
  //            if (node.left() instanceof AggregateExpressionNode leftAgg && node.right()
  // instanceof LiteralNode rightLiteral) {
  //                // 在聚合后的Schema中，聚合函数本身（如"AVG(salary)"）就是列名
  //                String aggColumnName = leftAgg.toString();
  //                int colIndex = schema.getColumnIndex(aggColumnName);
  //                if (colIndex == -1) {
  //                    throw new IllegalStateException("Aggregate column '" + aggColumnName + "'
  // not found in post-aggregation schema.");
  //                }
  //                Value leftValue = tuple.getValues().get(colIndex);
  //                Value rightValue = getLiteralValue(rightLiteral);
  //                return compareValues(leftValue, rightValue, node.operator().type().name());
  ...
  //            else if (node.left() instanceof LiteralNode leftLiteral && node.right() instanceof
  // AggregateExpressionNode rightAgg) {
  //                String aggColumnName = rightAgg.toString();
  //                int colIndex = schema.getColumnIndex(aggColumnName);
  //                if (colIndex == -1) {
  //                    throw new IllegalStateException("Aggregate column '" + aggColumnName + "'
  ...
  //            // 此处修改结束
  //            //
  //            if (node.left() instanceof IdentifierNode leftCol && node.right() instanceof
  // IdentifierNode rightCol) {
  //                Value leftValue = getColumnValue(tuple, schema, leftCol);
  //                Value rightValue = getColumnValue(tuple, schema, rightCol);
  //                return compareValues(leftValue, rightValue, node.operator().type().name());
  //            }
  //
  //            if (node.left() instanceof IdentifierNode columnNode && node.right() instanceof
  ...
  //            }
  //
  //            throw new UnsupportedOperationException("Expression format not supported. Must be
  // 'column op literal' or 'column1 op column2'.");
  //        }
  //        // 此处修改: 更新异常信息
  //        throw new UnsupportedOperationException("Unsupported expression type in WHERE or HAVING
  // clause: " + expression.getClass().getSimpleName());
  //    }
