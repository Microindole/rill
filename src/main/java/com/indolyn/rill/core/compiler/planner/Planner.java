...
public class Planner {
  ...
  //
  //        if (indexInfo != null) {
  //            System.out.println("[Planner] Index found for '" + fromTableName + "." +
  // indexInfo.getColumnName() + "'. Using Index Scan.");
  //            Value searchValue = extractValueFromPredicate(ast.whereClause());
  //            plan = new IndexScanPlanNode(fromTableInfo, indexInfo, searchValue);
  //        } else {
  //            System.out.println("[Planner] No suitable index found for query. Using Sequential
  // Scan.");
  ...
  //            if (ast.groupByClause() != null) {
  //                for (IdentifierNode groupByCol : ast.groupByClause()) {
  //                    outputColumns.add(findColumnInSchema(plan.getOutputSchema(),
  // groupByCol.getName()));
  //                }
  //            }
  ...
  //            for (ExpressionNode expr : ast.selectList()) {
  //                if (expr instanceof IdentifierNode idNode) {
  //                    Column originalColumn = findColumnInSchema(inputSchemaForProject,
  // idNode.getName());
