...
public class Page {
  ...
  /**
   * 计算剩余可用空间。 可用空间 = 空闲空间指针 - (页头大小 + 已有槽位大小)
   *
   * @return 剩余字节数
   */
  ...
  public Tuple getTuple(int slotIndex, Schema schema) {
    ...
    if (offset < HEADER_SIZE || offset + length > PAGE_SIZE) {
      System.err.println(
          "WARNING: Corrupted tuple slot found in Page "
              + pageId.getPageNum()
              + " at slot "
              + slotIndex
              + ". Invalid offset="
  ...
  /**
   * 撤销对一个元组的删除标记。 这是 markTupleAsDeleted 的逆操作。
   *
   * @param slotIndex 槽位号
  ...
  public boolean deleteTuple(int slotIndex) {
    ...
    if (slotToMoveSize > 0) {
      System.arraycopy(
          data.array(),
          HEADER_SIZE + (slotIndex + 1) * SLOT_SIZE,
          data.array(),
          HEADER_SIZE + slotIndex * SLOT_SIZE,
          slotToMoveSize);
    }
    // 4. 更新槽位偏移量
