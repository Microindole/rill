...
/**
 * B+树叶子节点页。 存储 <Key, RID> 对。 Header: [NodeType, KeyCount] Data: [NextLeafPageId, <Key1, RID1>,
 * <Key2, RID2>, ...]
 */
public class BPlusTreeLeafPage extends BPlusTreeNodePage {
  ...
  /**
   * 插入一个新的 <Key, RID> 对到叶子节点。 必须保证键的有序性。
   *
   * @param key 要插入的键
  ...
  public void insertAtFront(Value key, RID rid) {
    ...
    // 所有元素向后移动一位
    System.arraycopy(
        data.array(), getEntryOffset(0), data.array(), getEntryOffset(1), keyCount * ENTRY_SIZE);
    setEntry(0, key, rid);
    setKeyCount(keyCount + 1);
  }
