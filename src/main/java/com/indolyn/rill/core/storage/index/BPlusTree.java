...
public class BPlusTree {
    ...
    private void splitLeafNode(
        BPlusTreeLeafPage oldNode, BPlusTreeLeafPage newNode, List<Value> keys, List<RID> rids) {
    ...
    private void insertIntoParent(
        BPlusTreeNodePage leftChild, Value key, BPlusTreeNodePage rightChild) throws IOException {
        if (leftChild.getParentPageId() == -1) {
            ...
            newRoot.populate(
                leftChild.page.getPageId().getPageNum(), key, rightChild.page.getPageId().getPageNum());
        ...
        if (parentNode.getKeyCount() < parentNode.getMaxSize()) {
            ...
            Value promotedKey =
                splitInternalNode(
                    parentNode, key, rightChild.page.getPageId().getPageNum(), newInternalNode);
    ...
    private Value splitInternalNode(
        BPlusTreeInternalPage oldNode, Value newKey, int newChildId, BPlusTreeInternalPage newNode)
        throws IOException {
    ...
    public boolean delete(Value key) throws IOException {
        ...
        if (index >= leafNode.getKeyCount()
            || keyComparator.compare(leafNode.getKey(index), key) != 0) {
    ...
    private void redistribute(
        BPlusTreeNodePage fromNode, BPlusTreeNodePage toNode, BPlusTreeInternalPage parent)
        throws IOException {
    ...
    private void merge(
        BPlusTreeNodePage leftNode, BPlusTreeNodePage rightNode, BPlusTreeInternalPage parent)
        throws IOException {

        int rightNodeIndexInParent =
            parent.getChildIndexByPageId(rightNode.page.getPageId().getPageNum());
