...
import com.indolyn.rill.core.compiler.planner.plan.query.AggregatePlanNode;
import com.indolyn.rill.core.engine.ExpressionEvaluator;
import com.indolyn.rill.core.executor.TupleIterator;
import java.io.IOException;
import java.util.*;

/** 聚合执行器 这是一个阻塞执行器，它会拉取所有子节点的元组，在内存中进行哈希聚合，然后再向上层返回。 */
public class AggregateExecutor implements TupleIterator {
  ...
  private void init() throws IOException {
    ...
    while (child.hasNext()) {
      ...
      aggregationMap.computeIfAbsent(
          groupByKey,
          k -> {
      ...
      for (int i = 0; i < plan.getAggregates().size(); i++) {
        ...
        int intValue =
            (tupleVal != null && tupleVal.getValue() instanceof Integer)
                ? (Integer) tupleVal.getValue()
                : 0;
