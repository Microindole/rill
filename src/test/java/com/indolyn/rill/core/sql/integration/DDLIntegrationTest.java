package com.indolyn.rill.core.sql.integration;
...
/**
 * 专门用于测试DDL（数据定义语言）和SHOW命令的集成测试. 覆盖 CREATE, ALTER, DROP, SHOW 在各种成功和失败场景下的行为.
 */
public class DDLIntegrationTest {
    ...
    void testDatabaseOperations() {
        ...
        assertTrue(
            createDbResult.contains("Query OK."),
            "CREATE DATABASE should report success with 'Query OK.'.");
        ...
        assertTrue(
            dropDbResult.contains("Query OK."),
            "DROP DATABASE should report success with 'Query OK.'.");
    ...
    void testTableDDLFlow() {
        ...
        assertTrue(
            showResult1.contains("Query finished, 0 rows returned"),
            "Initially, SHOW TABLES should be empty.");
        ...
        assertTrue(
            showResult2.contains("users") && showResult2.contains("1 rows returned"),
            "SHOW TABLES should now list the 'users' table.");

        // 3. ALTER TABLE 添加新列并验证 (因为解析器不支持 ALTER，暂时注释掉这部分)
        // String alterResult = queryProcessor.executeAndGetResult("ALTER TABLE users ADD COLUMN age
        // INT;");
        ...
        // assertTrue(showColumnsResult.contains("age") && showColumnsResult.contains("int"), "SHOW
        // COLUMNS should display the new 'age' column.");
        ...
        assertTrue(
            showResult3.contains("Query finished, 0 rows returned"),
            "After dropping, SHOW TABLES should be empty again.");
    }
    ...
    void testDDLErrorHandling() {
        ...
        // 1. 尝试创建已存在的表
        String createError =
            queryProcessor.executeAndGetResult("CREATE TABLE existing_table (name VARCHAR);");
        assertTrue(
            createError.contains("ERROR: Table 'existing_table' already exists."),
            "Should fail when creating a table that already exists.");

        // 2. 尝试 ALTER 不存在的表 (因为解析器不支持 ALTER，暂时注释掉这部分)
        // String alterError = queryProcessor.executeAndGetResult("ALTER TABLE non_existent_table ADD
        // COLUMN score INT;");
        // assertTrue(alterError.contains("ERROR: Table 'non_existent_table' not found."), "Should fail
        // when altering a non-existent table.");

        // 3. 尝试添加已存在的列 (因为解析器不支持 ALTER，暂时注释掉这部分)
        // String addColumnError = queryProcessor.executeAndGetResult("ALTER TABLE existing_table ADD
        // COLUMN id INT;");
        // assertTrue(addColumnError.contains("ERROR: Column 'id' already exists"), "Should fail when
        // adding a column that already exists.");

        // 4. 尝试 DROP 不存在的表
        String dropError = queryProcessor.executeAndGetResult("DROP TABLE non_existent_table;");
        assertTrue(
            dropError.contains("ERROR: Table 'non_existent_table' not found."),
            "Should fail when dropping a non-existent table.");
    }
