...
public class PredicatePushdownPerformanceTest {
    ...
    void setUp() {
        ...
        for (int i = 1; i <= DATA_VOLUME; i++) {
            queryProcessor.execute(
                String.format(
                    "INSERT INTO perf_table (id, name, value) VALUES (%d, 'name_%d', %d);",
                    i, i, i * 10));
        }
    ...
    void testPredicatePushdownPerformanceGain() {
        ...
        System.out.println("\n[步骤 4] 性能对比与最终验证...");
        System.out.printf(
            "  - 性能提升倍数: %.2f 倍%n", (double) durationWithoutPushdown / durationWithPushdown);

        // 核心断言：带有谓词下推的查询必须比全量拉取快得多。
        assertTrue(
            durationWithPushdown < durationWithoutPushdown / 4,
            String.format(
                "谓词下推带来的性能提升未达到预期！无下推耗时: %d ms, 有下推耗时: %d ms",
