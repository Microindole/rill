...
/** BPlusTree 的最终完整功能测试类。 包含专门针对节点分裂的精确测试，以及大规模随机压力测试。 */
public class BPlusTreeTest {
  ...
  public void testMassiveInsertDeleteAndSearch() throws IOException {
    ...
    for (int key : initialKeys) {
      ...
      assertNotNull(
          foundRid, "Key " + key + " should be found after initial insertion, but was not.");
    ...
    for (int key : keysToDelete) {
      assertNull(
          bPlusTree.search(new Value(key)), "Deleted key " + key + " was found in the tree.");
    }
    ...
    System.out.println(
        "[Phase 5] Second insertion complete. Total keys now: " + presentKeys.size());
    ...
    for (int key : presentKeys) {
      RID foundRid = bPlusTree.search(new Value(key));
      assertNotNull(
          foundRid, "Key " + key + " should be present in the final tree, but was not found.");
    ...
    assertEquals(
        presentKeys.size(),
        verifiedCount,
        "The number of verified keys does not match the expected count.");
    System.out.println(
        "[Phase 6] Successfully verified " + verifiedCount + " keys. The B+ Tree is stable.");
  }
}
