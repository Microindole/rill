...
public class ConcurrencyTest {
    ...
    void setUp() throws IOException {
        ...
        diskManager = new DiskManager("data/" + TEST_DB_NAME + "/rill.data");
        diskManager.open();
        bufferPoolManager = new BufferPoolManager(100, diskManager, "LRU");
        logManager = new LogManager("data/" + TEST_DB_NAME + "/rill.data.log");
    ...
    void testWriterBlocksReader() throws InterruptedException {
            ...
            new Thread(
                () -> {
                    try {
                        ...
                        System.out.println(
                            "[Writer]  : Tries to acquire EXCLUSIVE lock on page " + pageId.getPageNum());
                        lockManager.lockExclusive(writerTxn, pageId);
                        System.out.println(
                            "[Writer]  : Acquired EXCLUSIVE lock. Notifying reader to start.");
            ...
            new Thread(
                () -> {
                    try {
                        ...
                        System.out.println(
                            "[Reader]  : Tries to acquire SHARED lock on page " + pageId.getPageNum());
        ...
        assertTrue(
            readerWasBlocked.get(), "Reader should have been blocked by the writer's exclusive lock.");
