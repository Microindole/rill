...
public class ParserTest {
  ...
  public void testParseCreateTable() {
    ...
    assertTrue(
        "AST node should be an instance of CreateTableStatementNode",
        node instanceof CreateTableStatementNode);
  ...
  public void testParseSimpleSelect() {
    ...
    StatementNode node = parseSql(sql);
    assertTrue(
        "AST node should be an instance of SelectStatementNode",
        node instanceof SelectStatementNode);
  ...
  public void testParsingWithoutTrailingSemicolon() {
    ...
    System.out.println(
        "Goal: Verify the parser can successfully handle a statement that ends at EOF without a semicolon.");
    ...
    try {
      ...
      assertNotNull(
          "Parser should successfully create an AST node even without a trailing semicolon at EOF.",
          node);
      assertTrue(
          "The parsed node should be a SelectStatementNode.", node instanceof SelectStatementNode);
    } catch (ParseException e) {
      // 如果抛出了异常，说明解析器的行为与我们的预期不符，测试失败。
      fail(
          "Parser threw an unexpected ParseException for a statement ending at EOF without a semicolon. Error: "
              + e.getMessage());
    }
