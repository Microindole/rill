// src/test/java/org/csu/rill/index/IndexPerformanceTest.java
    ...
    void testIndexPerformanceGain() {
        ...
        for (int i = 1; i <= DATA_VOLUME; i++) {
            queryProcessor.execute(
                String.format("INSERT INTO perf_test (id, name) VALUES (%d, 'user%d');", i, i));
        }
        ...
        String catalogQuery =
            "SELECT root_page_id FROM _catalog_indexes WHERE index_name = 'idx_perf_test_id';";
        ...
        assertTrue(
            durationWithIndex < durationWithoutIndex / 5,
            String.format(
                "索引性能提升未达到预期！无索引耗时: %d ms, 有索引耗时: %d ms", durationWithoutIndex, durationWithIndex));
