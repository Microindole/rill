// src/test/java/org/csu/rill/UpdateAndRecoveryFixTest.java
...
public class UpdateAndRecoveryFixTest {
    ...
    private final String TEST_DB_FILE = "data/" + TEST_DB_NAME + "/rill.data";
    private final String TEST_LOG_FILE = "data/" + TEST_DB_NAME + "/rill.data.log";
    ...
    void testPrimaryKeyViolationOnUpdateAndRecovery() throws IOException {
        ...
        assertTrue(
            result.contains("Primary key constraint violation"),
            "Update should be rejected due to PK violation.");
        ...
        assertFalse(
            finalResult.contains("'Alice'"), "Original name 'Alice' should NOT be present after redo.");
        assertTrue(finalResult.contains("Bob"), "Unaffected record 'Bob' should still be present.");
        assertTrue(
            finalResult.contains("2 rows returned"), "There should be exactly 2 rows after recovery.");
        ...
        assertEquals(
            1,
            countOfAlicia,
            "There should be only ONE record with the name 'Alicia'. Duplicates indicate a recovery bug.");
