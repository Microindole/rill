package com.indolyn.rill.core.replacement;
...
public class ClockReplacerTest {
  ...
  void testClockEvictionLogic() throws IOException {
    ...
    System.out.println(
        "Buffer pool filled. Page table size: " + bufferPoolManager.getPageTable().size());
    ...
    assertNotNull(
        bufferPoolManager.getPageTable().get(p1), "Page 1 should still be in the buffer pool.");
    assertNotNull(
        bufferPoolManager.getPageTable().get(p2), "Page 2 should still be in the buffer pool.");
