package com.indolyn.rill.core.execution.optimization;
 ...
 *
 * <p>该测试验证 WHERE 子句中的过滤条件是否被成功下推到扫描层执行， 从而提高 SELECT, UPDATE, DELETE 等操作的效率和正确性。
 */
public class PredicatePushdownTest {
    ...
    void setUp() {
        ...
        queryProcessor.execute(
            "CREATE TABLE products (id INT, name VARCHAR, category VARCHAR, stock INT);");
        // 插入多条记录，以便进行丰富的查询测试
        queryProcessor.execute(
            "INSERT INTO products (id, name, category, stock) VALUES (1, 'Apple', 'Fruit', 100);");
        queryProcessor.execute(
            "INSERT INTO products (id, name, category, stock) VALUES (2, 'Milk', 'Dairy', 50);");
        queryProcessor.execute(
            "INSERT INTO products (id, name, category, stock) VALUES (3, 'Bread', 'Bakery', 80);");
        queryProcessor.execute(
            "INSERT INTO products (id, name, category, stock) VALUES (4, 'Orange', 'Fruit', 120);");
        queryProcessor.execute(
            "INSERT INTO products (id, name, category, stock) VALUES (5, 'Cheese', 'Dairy', 30);");
    }
    ...
    /**
     * 测试场景 1: 基本的 SELECT 查询与谓词下推 验证 WHERE 子句是否能正确过滤数据。
     */
    @Test
    ...
    /**
     * 测试场景 2: SELECT 查询，但没有匹配的行 验证谓词下推是否能正确处理没有结果的情况。
     */
    @Test
    ...
    /**
     * 测试场景 3: 没有任何 WHERE 子句的 SELECT 验证在没有谓词需要下推时，系统是否能正常执行全表扫描。
     */
    @Test
    ...
    /**
     * 测试场景 4: UPDATE 操作与谓词下推 验证 WHERE 子句是否能定位到正确的行进行更新。
     */
    @Test
    ...
    /**
     * 测试场景 5: DELETE 操作与谓词下推 验证 WHERE 子句是否能定位到正确的行进行删除。
     */
    @Test
