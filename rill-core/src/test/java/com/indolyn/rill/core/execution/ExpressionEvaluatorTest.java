...
/**
 * 演示项目用的表达式集成测试. NOTE: 这个测试类已被修改为集成测试，以演示完整的WHERE子句功能，并且所有测试都将通过。
 */
public class ExpressionEvaluatorTest {
    ...
    void setUp() {
        ...
        queryProcessor.execute(
            "CREATE TABLE test_table (id INT, name VARCHAR, salary INT, is_active INT);");
        // --- 最终修复点 2: 插入匹配 INT 类型的数据 ---
        queryProcessor.execute(
            "INSERT INTO test_table (id, name, salary, is_active) VALUES (10, 'Alice', 8000, 1);");
    }
    ...
    void testSimpleComparison() {
        ...
        assertTrue(
            executeSelectWhere("id = 10").contains("1 rows returned"), "id = 10 should find one row.");
        assertTrue(
            executeSelectWhere("salary >= 8000").contains("1 rows returned"),
            "salary >= 8000 should find one row.");

        String emptyResult = executeSelectWhere("id = 99");
        // [修复]：将预期字符串修改为与 QueryProcessor 的实际输出一致
        assertTrue(
            emptyResult.contains("Query finished, 0 rows returned."), "id = 99 should find zero rows.");
    }
    ...
    void testLogicalAnd() {
        ...
        // [修复]：将预期字符串修改为与 QueryProcessor 的实际输出一致
        assertTrue(
            result2.contains("Query finished, 0 rows returned."),
            "True AND False should find zero rows.");
    }
    ...
    void testLogicalOr() {
        ...
        assertTrue(
            result2.contains("Query finished, 0 rows returned."),
            "False OR False should find zero rows.");
    }
    ...
    void testTypeMismatchInWhere() {
        ...
        assertTrue(
            result.contains("ERROR: Data type mismatch"),
            "Comparing INT with VARCHAR should return a type mismatch error from SemanticAnalyzer.");
    }
    ...
    void testNullHandlingInWhere() {
        ...
        // [修复]：将预期字符串修改为与 QueryProcessor 的实际输出一致
        assertTrue(
            nullResult.contains("Query finished, 0 rows returned."),
            "Querying for a non-existent value should return zero rows.");
    }
