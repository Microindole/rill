...
当前 `rill-core` 旧测试目录已经整体下线，但新的测试体系已经不再是空白状态。

当前已重建并纳入 `verify` 的测试层包括：

- 基础设施：schema、database manager、log record、log manager、lock manager、transaction manager
- 存储：page、disk manager、buffer pool、replacement、B+Tree、table heap
- 编译器：lexer/parser、类型解析、命令规划
- 执行层：结果渲染、内建命令、表名解析、命令执行器、表达式与谓词辅助
- 集成：数据库命令、元数据命令、DML、恢复、查询特性、权限命令

当前默认 CI 在原有多平台 `package`、脚本 smoke test 和前端 `build` 之外，还会单独上报 `rill-core` 覆盖率到 Codecov，但不设置失败门槛。

也就是说，当前仓库仍处于“测试体系继续重建中”，但已经不再是“测试缺席”状态。

## 新测试架构方向

后续测试不再按历史遗留目录随意堆放，而是按两条线同时收口：

### 1. 由内到外

- 基础设施：页、tuple、schema、buffer、disk、日志记录编解码
- 存储与事务：table heap、B+Tree、锁、事务、恢复
- 编译器：lexer、parser、semantic、planner、类型系统
- 执行层：DDL / DML / query 的行为验证
- 通信层：native server、MySQL 协议、CLI、Web API
- 集成测试：真实请求穿透整条链路

### 2. 由外到内

- 以真实 SQL、真实请求、真实恢复场景为入口
- 反向压到编译器、执行器、存储、日志、锁
- 用少量但关键的端到端场景锁住系统行为
...
- 基础设施层要小、快、可精确定位
- 存储/事务层要验证内核不变量，不依赖外层协议
- 编译器层要聚焦 AST、语义、计划，不混入真实网络通信
- 执行层要验证行为，不重复深测协议格式
- 通信层只测协议/适配，不重复内核细节
- 集成测试数量要少，但必须覆盖关键主链路
