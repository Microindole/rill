...
当前 `rill-core` 旧测试目录已经整体下线。

原因不是“项目不再需要测试”，而是：

- 旧测试长期跨越多轮重构，很多断言已经脱离当前代码结构
- 部分测试粒度和边界混乱，把基础设施、索引性能、系统集成混在一起
- 部分测试成本过高，尤其是索引/性能类测试，不适合作为默认 CI 基线

当前默认 CI 因此只保留：

- 多平台 `package`
- 启动脚本与统一命令入口 smoke test
- 前端 `npm run build`

也就是说，当前仓库处于“测试体系重建中”，不是“测试体系已经完成”。
