...
Web UI 不是项目介绍页，也不是 GUI 的浏览器翻版，而是数据库内核的演示台与可观测性控制台。
...
- 让面试或演示时不只看到“能跑 SQL”
- 还能讲清模块结构、能力边界、发布形态和后续扩展方向
- 把网络编程、Redis、未来分布式预留这类话题有位置可讲，而不是只停在口头描述
...
- `System Architecture`：展示多模块结构、产品边界和各模块职责
- `Capability Panels`：展示可对外讲解的能力，不只局限在 SQL
- `Expansion Roadmap`：展示网络编程、Redis、分布式预留等后续方向
...
- 已支持 `GET /api/overview`，用于返回模块结构、能力摘要和扩展路线
- 当前接口返回真实 SQL 执行结果、阶段级 trace 与原始文本结果
- 当前前端在后端不可用时会回退到 mock trace，便于独立开发
- 当前前端在后端不可用时也会回退到 mock overview，保证演示台结构可独立开发
...
- 当前 Web UI 已开始从“一页 SQL 控制台”升级为“SQL + 架构 + 能力 + 扩展路线”的演示台
- 当前 Web 后端查询、trace 和 overview 链路已经有对应单测，演示台不再只靠手工点击验证
