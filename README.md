...
仓库当前已增加 `main` 分支自动部署 workflow：

- 文件：`.github/workflows/deploy-main.yml`
- 触发条件：`push` 到 `main` 或手动触发
- 部署方式：GitHub Actions 通过 SSH 登录服务器并执行 `/home/indolyn/deploy-rill.sh`

服务器侧前置条件：

- 仓库已部署在 `/home/indolyn/rill`
- `rill-app-web` 已由 `systemd` 管理
- 本地敏感配置位于 `/home/indolyn/rill/config/rill-app-secrets.properties`
- `/home/indolyn/deploy-rill.sh` 已具备执行权限，并负责：
  - `git fetch`
...
- `DEPLOY_USER`：SSH 登录用户，例如 `indolyn`
- `DEPLOY_SSH_KEY`：用于部署的私钥
- `DEPLOY_PORT`：可选，默认 `22`

如果部署脚本里需要 `sudo systemctl restart rill-app-web`，还需要在服务器上为部署用户配置最小范围的免密 sudo。
