...
- `rill-app-web`: Spring Boot Web 壳，供 `web/` 前端调用
...
仓库当前已增加 tag 驱动的发布流水线。推送 `v*` tag 后，会自动生成并上传以下产物到 GitHub Release：

- Windows 安装包：内置精简 Java runtime，固定安装数据库内核服务端与 CLI，可选安装 GUI 和 MySQL/Navicat 兼容服务端
- Linux 归档包：内置精简 Java runtime，包含数据库内核服务端、CLI、GUI、MySQL/Navicat 兼容服务端及 Linux 启动脚本
- macOS 归档包：内置精简 Java runtime，包含数据库内核服务端、CLI、GUI、MySQL/Navicat 兼容服务端及 macOS 启动脚本
- 纯 Spring Boot jar：只包含 `rill-app-web` 后端，不内嵌前端静态资源
- 带前端静态资源的 Spring Boot jar：先构建 `web/dist`，再打入 `rill-app-web` 的 `static/`

当前 Windows 安装包的组件边界为：
...
- `rill-app-web-<tag>.jar`：适合前后端分离部署
- `rill-app-web-<tag>-with-ui.jar`：适合单文件部署 Web 控制台

当前桌面/本地发布边界为：

- `rill-<tag>-windows-x64-setup.exe`
- `rill-<tag>-linux-x64.tar.gz`
- `rill-<tag>-macos-arm64.tar.gz`
...
`com.indolyn.rill.app.boot.RillLauncher` 仍保留为 IDE / 开发期统一入口；正式打包分发改为模块化工件。`server/mysql-server/client/gui` 由各自模块独立打包，Spring Boot Web 壳单独位于 `rill-app-web`。
